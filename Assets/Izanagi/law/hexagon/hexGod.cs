...
public class hexGod : MonoBehaviour
{
    ...
    void GenerateTile(Transform _gridPop, Transform _signPop, int _token, int _countX, int _countZ)
    {
        ...
        {
            Vector3 table = new Vector3(_countX, 0, _countZ);
            tile.DefineTile(schematic, _token, table, defaultBiome);
        }
        ...
        Text sign = Instantiate(signCell);
        {
            ...
            sign.text = hexAid.DefineSign(tile.axial);
        }
    }
    ...
    // Accessed to generates tile biome.
